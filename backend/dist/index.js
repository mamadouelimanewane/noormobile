...
app.post('/api/carpool/book', async (req, res) => {
    try {
        ...
        if (availableSeats < placesToBook)
...
app.post('/api/loans/request', async (req, res) => {
    try {
        ...
        if (existing)
            return res.status(400).json({ error: 'Vous avez déjà une demande en attente ou un prêt en cours.' });
        const settings = await prisma.platformSettings.findUnique({ where: { id: 'default' } });
        const maxLoan = settings?.maxLoanAmount || 100000;
        if (montant > maxLoan)
            return res.status(400).json({ error: `Le montant ne peut excéder ${maxLoan} FCFA.` });
...
app.post('/api/admin/loans/approve', async (req, res) => {
    try {
        ...
        if (!loan || loan.status !== 'en_attente')
            return res.status(400).json({ error: 'Prêt introuvable ou déjà traité' });
        await prisma.$transaction([
...
io.on('connection', (socket) => {
    ...
    socket.on('ride:status', async (data) => {
        ...
        if (data.status === 'termine' && req.driverId) {
            ...
            if (driver) {
                if (driver.driverLevel === 'SILVER')
                    commRate = 0.10;
                if (driver.driverLevel === 'GOLD')
                    commRate = 0.08;
                const newRidesCount = driver.completedRides + 1;
                let newLevel = 'BRONZE';
                if (newRidesCount > 50)
                    newLevel = 'SILVER';
                if (newRidesCount > 200)
                    newLevel = 'GOLD';
                await prisma.user.update({
